...
<Summary>
增量运算符`(++)`给其操作数加 1 并返回一个值。类似地，减量运算符`(--)`从其操作数中减去 1 并返回一个值。这两个运算符都可以用作前缀(`++i`，`--i`)或后缀(`i++`，`i--`)。

如果使用前缀，值会先增加/减少，然后表达式的值是更新后的值。
...
这种区别在实际编程中很重要，特别是在复杂表达式或循环中使用这些运算符时。正确理解和使用前缀和后缀运算符可以帮助避免意外的行为和 bug。
...
这里使用 `i++` 是常见做法，因为我们通常想在每次迭代后增加 `i`，而不是在使用它之前。

然而，在某些情况下，使用前缀运算符可能更合适：
...
这个循环会打印 1 到 4，因为 i 在比较之前就增加了。

理解这些细微的差别可以帮助开发者写出更精确和高效的代码。
</Summary>
