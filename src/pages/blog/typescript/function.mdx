...
跟 JavaScript 定义函数十分相似, 可以通过 `funciton` 关键字、`箭头函数`等形式去定义, 例如下面一个简单的加法函数: 

```ts showLineNumbers copy
const add = (a: number, b: number): number => a + b
```

上述只定义了函数的两个参数类型, 这个时候整个函数虽然没有被显式定义, 但是实际上TypeScript 编译器是能够通过类型推断到这个函数的类型
...
这时候参数b可以是`number`类型或者`undefined`类型, 即可以传一个`number`类型或者不传都可以
