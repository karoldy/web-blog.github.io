...
在 `JavaScript` 中 `null` 表示 "什么都没有", 是一个只有一个值的特殊类型, 表示一个空对象引用, 而 `undefine`d 表示一个没有设置值的变量
...
但是 `TypeScript` 配置了 `--strictNullChecks` 标记, `null` 和 `undefined` 只能赋值给 `void` 和它们各自本身
