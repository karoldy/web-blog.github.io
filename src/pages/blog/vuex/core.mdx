...
存储在`Vuex`中的数据和`Vue`实例中的`data`遵循相同的规则, 例如状态对象必须是纯粹`(plain)`的。参考：[Vue#data](https://cn.vuejs.org/api/options-state#data-2)。
...
而且, 你可以通过使用`createNamespacedHelper` 创建基于某个命名空间辅助函数。它返回一个对象, 对象里有新的绑定在给定命名空间值上的组件绑定辅助函数：
