...
async function svgFileToMuiComponent(
    ...
    finalSvg,
    {
      ...
      template: (variables, { tpl }) => {
        return tpl`
          ...
          const ${variables.componentName}: React.FC<SvgIconProps> = (props) => {
            return (
              ${variables.jsx}
            );
          }
  ...
  const code = jsCode
    .replace(
      /<svg[^>]*?(?:xmlns=\"[^\"]*\"|width=\{[^}]*\}|height=\{[^}]*\}|fill=\"[^\"]*\")[^>]*({\.\.\.props}[^>]*)?>/m,
      '<SvgIcon inheritViewBox {...props}>'
    )
    .replace(/<\/svg>/g, '</SvgIcon>');
  await writeFileIfChanged(outFile, code);
