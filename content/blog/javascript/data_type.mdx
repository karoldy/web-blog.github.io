...
`Undefined` 类型只有一个值, 就是特殊值 `undefined`。当使用 `var `或 `var `声明了变量但没有初始化时, 就相当于给变量赋予了 `undefined `值
...
```js showLineNumbers copy
var message; // 这个变量被声明了, 只是值为 undefined
...
var firstName = "John";
var lastName = 'Jacob';
var lastName = `Jingleheimerschmidt`
```
...
```js showLineNumbers copy
var genericSymbol = Symbol();
var otherGenericSymbol = Symbol();
console.log(genericSymbol == otherGenericSymbol); // false

var fooSymbol = Symbol('foo');
var otherFooSymbol = Symbol('foo');
